...
extension OptionsGlobMatch on Map<String, OptionConfig> {
  bool? _hasGlobMatch(String option, String filePath) {
    final String? isEnabled = this[option]?.enabled.firstWhereOrNull(
      (String glob) => Glob(glob).matches(filePath),
    );
    ...
    final String? isDisabled = this[option]?.disabled.firstWhereOrNull(
      (String glob) => Glob(glob).matches(filePath),
    );
  ...
  bool effectiveUnmodifiableCollections({
    required String filePath,
    required bool defaultValue,
  }) {
  ...
  bool effectiveToJsonDropNullValues({
    required String filePath,
    required bool defaultValue,
  }) {
  ...
  bool _effectiveValue(String method, String filePath, bool defaultValue) {
