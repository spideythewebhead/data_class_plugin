...
class CopyWithGenerator implements Generator {
  ...
  Future<void> _writeCopyWithProxyClass() async {
    _codeWriter
      ..writeln()
      ..writeln(
        'abstract interface class _${_className}CopyWithProxy$_classTypeParametersSource {',
      )
      ..writeln();
    ...
    for (final DeclarationInfo field in _fields) {
      ...
      if (!dartType.isPrimitive &&
        ...
        if (match != null && node is ClassDeclaration && node.hasDataClassAnnotation) {
          final String relativeFilePath = relative(
            match.filePath,
            from: _projectDirectoryPath,
          );
          useCopyWithProxy =
              AnnotationValueExtractor(
                node.dataClassAnnotation,
              ).getBool('copyWith') ??
  ...
  Future<void> _writeCopyWithProxyImplClass() async {
    ...
    _codeWriter
      ...
      ..writeln()
      ..writeln(
        'final $_className$_classTypeParametersWithoutConstraints _value;',
      )
      ..writeln();
    ...
    for (final DeclarationInfo field in _fields) {
      ...
      if (useCopyWithProxy) {
        _codeWriter
          ...
          ..write(' get $fieldName => ')
          ..write(
            dartType.isNullable ? '_value.$fieldName == null ? null : ' : '',
          )
          ..writeln(
          ...
          ..write(dartType.isNullable ? '!' : '')
          ..writeln(
            ', (${dartType.fullTypeName} update) => this($fieldName: update));',
          )
          ..writeln();
        continue;
      }
    ...
    for (final DeclarationInfo field in _fields) {
      ...
      if (dartType.isNullable) {
        _codeWriter
          ...
          ..write(
            ' ? _value.$fieldName : ($fieldName as ${dartType.fullTypeName}),',
          );
      } else {
  ...
  Future<void> _writeCopyWithProxyChainClass({
    ...
    _codeWriter
      ..writeln()
      ..writeln(
        'sealed class \$${_className}CopyWithProxyChain$proxyChainGenericArgs {',
      );
  ...
  Future<void> _writeCopyWithProxyChainImplClass({
    ...
    _codeWriter
      ..writeln(
        '_${_className}CopyWithProxyChainImpl(this._value, this._chain);',
      )
      ..writeln()
      ..writeln(
        'final $_className$_classTypeParametersWithoutConstraints _value;',
    ...
    _codeWriter
      ..writeln(') {')
      ..writeln(
        'return _chain($_generatedClassName$_classTypeParametersWithoutConstraints(',
      );
