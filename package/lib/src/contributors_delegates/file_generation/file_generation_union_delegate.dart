...
class FileGenerationUnionDelegate extends ClassGenerationDelegate {
    ...
    await changeBuilder.addDartFileEdit(targetFilePath, (
      ...
      for (final ClassDeclaration classNode in classNodes) {
        ...
        final UnionInternal unionInternalAnnotation = UnionInternal.fromDartObject(
          unionElementAnnotation.computeConstantValue(),
        );
        ...
        final RedirectedConstructorsVisitor redirectedConstructorsVisitor =
            RedirectedConstructorsVisitor(
              result: <String, RedirectedConstructor>{},
            );
  ...
  void _generateFromJsonFunction({
    ...
    if (sourceRange != null) {
      fileEditBuilder.addReplacement(
        sourceRange,
        (DartEditBuilder builder) => _writeFromJsonFunction(
          classElement: classElement,
          builder: builder,
        ),
      );
  ...
  void _generateToJsonFunction({
    ...
    void writerToJsonFunction(DartEditBuilder builder) {
      _writeToJsonFunction(classElement: classElement, builder: builder);
    }

    if (sourceRange != null) {
      fileEditBuilder.addReplacement(sourceRange, writerToJsonFunction);
    } else {
      fileEditBuilder.addInsertion(
