...
''');
        ...
        expect(
          options.union.optionsConfig['data_class']?.defaultValue,
          equals(false),
        );
        expect(
          options.union.optionsConfig['from_json']?.defaultValue,
          equals(true),
        );
        expect(
          options.union.optionsConfig['to_json']?.defaultValue,
          equals(true),
        );
      });
...
''');
        ...
        for (final String option in <String>[
          ...
          expect(options.union.optionsConfig[option]?.enabled, hasLength(1));
          expect(
            options.union.optionsConfig[option]?.enabled.first,
            equals(enabledPath),
          );

          expect(options.union.optionsConfig[option]?.disabled, hasLength(1));
          expect(
